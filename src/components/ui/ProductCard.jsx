...
function ProductCard({ product, onCompare }) {
  return (
    <article className="card-lift rounded-xl border border-white/10 bg-card p-6 text-navy shadow-xl">
      ...
      <p className="mt-2 text-sm text-neutral-600">{product.short}</p>
      ...
      <div className="mt-6 flex gap-2">
        ...
        <button onClick={() => onCompare(product)} className="rounded-md border border-neutral-300 px-3 py-2 text-xs font-semibold hover:border-accent hover:text-accent">Compare</button>
      </div>
    </article>
  );
}
